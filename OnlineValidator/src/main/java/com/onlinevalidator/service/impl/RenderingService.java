...
public class RenderingService implements RenderingServiceInterface {
	 ...
	 * @param rapportoValidazione è la rappresentazione JAXB dell'XSD di
	 *                            riferimento
	 */
	...
	/**
	 * Funzione che si occupa di comporre interamente il documento PDF, che
	 * contiene il risultato di validazione.
	 *
	 * @param pdfDocument      è il documento PDF in via di composizione
	 * @param validationReport è il report di validazione di cui occorre
	 *                         effettuare il rendering
	 * @throws DocumentException nel caso in cui qualcosa vada storto durante il
	 *                           rendering
	 */
	private void composePdfFile(Document pdfDocument, ValidationReport validationReport) throws DocumentException {
		...
		pdfDocument.add(
				new Paragraph(
						String.format(
								"Rapporto di validazione (versione validatore %s)",
								StringUtils.isEmpty(validationReport.getVersioneValidatore())
										? "n/d" : validationReport.getVersioneValidatore()
						),
		...
		if (!StringUtils.isEmpty(validationReport.getTipoDocumentoValidato())) {
			pdfDocument.add(
					new Paragraph(
							String.format(
									"Tipo documento selezionato: %s; Formato: %s",
									validationReport.getTipoDocumentoValidato(),
									validationReport.getFormatoDocumentoValidato()
							),
		...
		if (!StringUtils.isEmpty(validationReport.getDescrizioneErroreXsd())) {
			...
			pdfDocument.add(
					new Paragraph(
							"Si è verificato un errore in sede di validazione xsd.",
							new Font(pdfMainFont, 11f, Font.NORMAL, BaseColor.BLACK)
					)
			);
	...
	/**
	 * Funzione che si occupa di comporre i paragrafi che dettagliano il
	 * contenuto del pdf.
	 *
	...
	/**
	 * Funzione che si occupa di scrivere un paragrafo all'interno del documento
	 * PDF.
	 *
	...
	/**
	 * Costruisce il nome di un file di esportazione partendo dal formato di
	 * riferimento.
	 *
	 * @param tipoRenderingEnum è la tipologia di rendering che si sta
	 *                          effettuando
